...
func TestRepository_ListCredentialLibraries_Pagination(t *testing.T) {
	...
	for _, cs := range css[:2] {
		page1, err := repo.ListCredentialLibraries(ctx, cs.GetPublicId(), WithLimit(2))
		require.NoError(err)
		require.Len(page1, 2)
		page2, err := repo.ListCredentialLibraries(ctx, cs.GetPublicId(), WithLimit(2), WithStartPageAfterItem(page1[1].GetPublicId(), page1[1].UpdateTime.AsTime()))
		require.NoError(err)
		require.Len(page2, 2)
		...
		page3, err := repo.ListCredentialLibraries(ctx, cs.GetPublicId(), WithLimit(2), WithStartPageAfterItem(page2[1].GetPublicId(), page2[1].UpdateTime.AsTime()))
		require.NoError(err)
		require.Len(page3, 1)
		for _, item := range append(page1, page2...) {
			assert.NotEqual(item.GetPublicId(), page3[0].GetPublicId())
		}
		page4, err := repo.ListCredentialLibraries(ctx, cs.GetPublicId(), WithLimit(2), WithStartPageAfterItem(page3[0].GetPublicId(), page3[0].UpdateTime.AsTime()))
		require.NoError(err)
		require.Empty(page4)
	}

	emptyPage, err := repo.ListCredentialLibraries(ctx, css[2].GetPublicId(), WithLimit(2))
	require.NoError(err)
...
func TestListDeletedIds(t *testing.T) {
	...
	for _, cs := range css[:2] { // Leave the third store empty
		libs = append(libs, TestCredentialLibraries(t, conn, wrapper, cs.GetPublicId(), 2)...)
		sshLibs = append(sshLibs, TestSSHCertificateCredentialLibraries(t, conn, wrapper, cs.GetPublicId(), 2)...)
	}
	...
	// Expect two entries for the first store
	deletedIds, err = repo.ListDeletedIds(ctx, time.Now().AddDate(-1, 0, 0))
	require.NoError(err)
...
func TestGetTotalItems(t *testing.T) {
	...
	_, err = repo.DeleteSSHCertificateCredentialLibrary(ctx, prj.GetPublicId(), sshLibs[2].GetPublicId())
	require.NoError(err)
	_, err = sqlDb.ExecContext(ctx, "analyze")
	require.NoError(err)
...
func TestNow(t *testing.T) {
	...
	sche := scheduler.TestScheduler(t, conn, wrapper)
	ctx := context.Background()
	repo, err := NewRepository(ctx, rw, rw, kmsRepo, sche)
	require.NoError(t, err)
