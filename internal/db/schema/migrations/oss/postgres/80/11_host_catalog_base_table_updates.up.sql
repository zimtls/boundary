...
begin;
  ...
  update host_catalog
    set create_time = sgc.create_time, update_time = sgc.update_time
    from host_catalog as hc
    left join static_host_catalog as sgc on hc.public_id = sgc.public_id;
  update host_catalog
    set create_time = hpc.create_time, update_time = hpc.update_time
    from host_catalog as hc
    left join host_plugin_catalog as hpc on hc.public_id = hpc.public_id;
  ...
  create index host_catalog_create_time_public_id_idx
      on host_catalog (create_time desc, public_id asc);
  create index host_catalog_update_time_public_id_idx
      on host_catalog (update_time desc, public_id asc);
