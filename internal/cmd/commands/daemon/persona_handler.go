...
func newPersonaHandlerFunc(ctx context.Context, repo *cache.Repository, refresher refresher) (http.HandlerFunc, error) {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		sAuthTokId, err := repo.LookupStoredAuthTokenId(ctx, perReq.BoundaryAddr, perReq.TokenName, perReq.KeyringType)
		if err != nil {
			writeError(w, "error performing stored token lookup", http.StatusInternalServerError)
			return
		}

		if err = repo.AddStoredToken(ctx, perReq.BoundaryAddr, perReq.TokenName, perReq.KeyringType, perReq.AuthTokenId); err != nil {
			writeError(w, "Failed to add a stored token", http.StatusInternalServerError)
			return
		}

		w.WriteHeader(http.StatusNoContent)

		if sAuthTokId != perReq.AuthTokenId {
